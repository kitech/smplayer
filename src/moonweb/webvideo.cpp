...
void WebVideo::library()
{
    ...
    {
        provider = pv;
        this->type = mv ? MP_TYPE_MOV : MP_TYPE_TV;
        keyword = type;
        downSearchPage();
    }
}
...
void WebVideo::downSearchPage()
{
    ...
    // call plugin
    if (type == MP_TYPE_ALBUM)
        plugins[provider]->searchAlbum(keyword, 1);
    else if (type == MP_TYPE_MOV || type == MP_TYPE_TV)
        plugins[provider]->library(type == MP_TYPE_MOV, keyword, 1);
    else
...
void WebVideo::nextSearchPage()
{
    ...
    nextButton->setEnabled(true);
    //call plugin
    if (type == MP_TYPE_TV || type == MP_TYPE_MOV)
        plugins[provider]->library(type == MP_TYPE_MOV, keyword, page_n);
    else if (type == MP_TYPE_ALBUM)
